...
const NAV_LINK =
  'flex cursor-pointer items-center gap-1 px-2 py-1 text-[10px] font-mono text-secondary hover:text-accent rounded hover:bg-elevated transition-colors'
const MOBILE_LINK =
  'flex items-center gap-2 py-3 text-lg font-mono font-semibold text-primary hover:text-accent transition-colors'
          ...
          <SiteFrame className="flex h-8 items-center justify-between">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <nav className="hidden lg:flex items-center gap-0.5">
                <div className="relative" ref={dropdownRef}>
                  ...
                  {dropdownOpen && (
                    <div className="absolute top-full left-0 mt-1 w-56 bg-card border border-border rounded-lg shadow-lg p-1 z-50">
                      <Link
                        href="/directorio"
                        className="block rounded-md px-3 py-2 text-xs font-mono text-secondary hover:text-accent hover:bg-elevated transition-colors"
                      >
                        Ver todo
                      </Link>
                      ...
                      {categories.map((cat) => {
                        ...
                        return (
                          <Link
                            key={cat.type}
                            href={`/${cat.slug}`}
                            className="flex items-center gap-2 rounded-md px-3 py-2 text-xs font-mono text-secondary hover:text-accent hover:bg-elevated transition-colors"
                          >
                ...
                <Link href="/eventos" className={NAV_LINK}>
                  <CalendarDays className="w-3 h-3" />
                  Eventos
                </Link>
                <Link href="/noticias" className={NAV_LINK}>
            ...
            <div className="flex lg:hidden items-center gap-1">
              ...
              >
                <Menu
                  className={`w-4 h-4 absolute transition-all duration-300 ${mobileOpen ? 'opacity-0 rotate-90 scale-75' : 'opacity-100 rotate-0 scale-100'}`}
                />
                <X
                  className={`w-4 h-4 absolute transition-all duration-300 ${mobileOpen ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 -rotate-90 scale-75'}`}
                />
              </button>
            </div>
          </SiteFrame>
      ...
      >
        <div className="flex items-center justify-between px-8 py-6">
          ...
          <button
            ...
            className="flex min-h-11 min-w-11 cursor-pointer items-center justify-center p-2 text-secondary hover:text-accent transition-colors"
        ...
        <div className="flex-1 flex flex-col justify-center px-8 space-y-2 uppercase">
          <Link href="/directorio" className={MOBILE_LINK}>
          ...
          <Link href="/eventos" className={MOBILE_LINK}>
            <CalendarDays className="w-5 h-5" />
            Eventos
          </Link>
          <Link href="/noticias" className={MOBILE_LINK}>
