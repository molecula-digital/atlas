    ...
    <>
      ...
      {!interactionEnabled && (
        <button
          ...
          className={buttonVariants({
            size: 'md',
            className:
              'absolute bottom-4 left-1/2 -translate-x-1/2 z-10 text-muted backdrop-blur-sm',
          })}
        >
          <Lock size={12} />
      ...
      <div className="absolute bottom-4 right-4 flex flex-col gap-1.5">
        <button
          onClick={onToggleLock}
          className={buttonVariants({
            variant: interactionEnabled ? 'accent' : 'neutral',
            size: 'icon-lg',
            className: 'rounded-lg',
          })}
          aria-label={interactionEnabled ? 'Bloquear mapa' : 'Desbloquear mapa'}
          title={
